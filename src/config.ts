...
const configSchema = z.object({
  ...
  T3N_AGENT_API_KEY: z.string().min(10).optional(),
  T3N_PRIVATE_MAP: z.string().min(1).default('incident-triage-private'),
  T3N_ENVIRONMENT: z.enum(['sandbox', 'testnet', 'production']).default('sandbox'),
});
...
export function loadRuntimeConfig(env: NodeJS.ProcessEnv = process.env): RuntimeConfig {
  ...
  return {
    ...
    ...((value.T3N_AGENT_API_KEY ?? value.T3N_API_KEY) ? { apiKey: value.T3N_AGENT_API_KEY ?? value.T3N_API_KEY } : {}),
  };
}
